...
#+begin_comment
This org file contains all the code needed to define an R package.  Use the Orgmode export engine with ox-ravel to ravel to Rmarkdown (.Rmd; C-c C-e r m). In R, run `litr::render("name-of-this-file.Rmd")` to generate the R package.  Remember that when you want to modify anything about the R package, you should modify this document rather than the derived .Rmd or package.

Code chunks that do not start with `#'` are not added to the package.

Code chunks that have one or more lines starting with `test_that(` (or `testthat::test_that(`) are added to the package as tests.
#+end_comment

* Package tasks [8/8]                                              :noexport:

** DONE Define default analysis pathway as a function
CLOSED: [2026-02-28 Sat 20:13]
- User should be able to supply just data, layout, bg setup, plot title.
- User should have access to
  - outlier iter
  - background-subtraction basis:  mean/median
  - background-subtraction options:  n points required
  - outlier plotting options:  drop/pointless/include, n.out threshold
  - background-subtraction plotting option:  show raw or corrected in test wells?
  - ignore-plotting option
- Path goes
  - read in data and interval length
  - set background groups
  - detect outliers in blanks
  - masking blank-outliers, aggregate blank vals for background subtraction
  - apply background subtraction, setting descriptive flags
  - detect outliers in background-subtracted test wells
  - plot and save
  
** DONE Add plot annotations tracking processing choices
CLOSED: [2026-02-28 Sat 21:20]
- [X] plot title
- [X] mean/median background aggregation
- [X] outliers included or excluded, individually and at what n.out threshold
- [X] status of ignore group

** DONE Update background inputs for pipeline
CLOSED: [2026-02-27 Fri 00:21]
- [X] make_bg_file with header lines
- [X] background_by_treatment with named list
- [X] background_by_wells making use of header lines rather than params

** DONE Check :: consistency throughout
CLOSED: [2026-02-28 Sat 22:55]

** DONE Check documentation throughout
CLOSED: [2026-03-01 Sun 13:48]

** DONE Build and install
CLOSED: [2026-03-01 Sun 14:52]
- ox-ravel export to Rmd
- fresh R session, litr::render()

** DONE Add README
CLOSED: [2026-03-01 Sun 21:56]
** DONE Push to github
CLOSED: [2026-03-20 Fri 15:48]
...
#+name: package-setup
#+begin_src R :ravel message=FALSE results="hide" :ravel include=FALSE
...
** Set up the R session for interactive development               :noexport:
#+name session-setup
#+begin_src R :session strato :exports none :results silent
# used in interactive development; not exported to .Rmd
require("dplyr")
require("tidyr")
require("readxl")
require("readr")
...
Optionally, you can also supply a template file, also in .csv format.  To reduce mistakes in setting up the template, this file should be shaped like a 96-well plate itself, with 8 rows (A-H) and 12 columns (1-12).  Each well's cell should hold the treatment group that well belongs to (e.g., "Control", "Blank", "BG.Cab_ATC").  To set up a plate layout file for a new plate, use =make_layout_file()= to write a fresh template to .xlsx or .csv for editing.

#+begin_src R :session strato :results silent :ravel include=FALSE
