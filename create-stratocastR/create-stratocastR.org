...
Our README.Rmd lives in the `source-files` directory.  We add `README.Rmd` to the package and then generate the `README.md` based on it:
...
We finish by running commands that will document and install the package.
