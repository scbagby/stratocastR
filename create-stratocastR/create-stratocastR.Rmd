...
Optionally, you can also supply a template file, also in .csv format.  To reduce mistakes in setting up the template, this file should be shaped like a 96-well plate itself, with 8 rows (A-H) and 12 columns (1-12).  Each well's cell should hold the treatment group that well belongs to (e.g., "Control", "Blank", "BG.Cab\_ATC").  To set up a plate layout file for a new plate, use \`make\_layout\_file()\` to write a fresh template to .xlsx or .csv for editing.
...
-   Outlier detection:  How many iterations should the \`forecast::tsoutlier()\` outlier-detection algorithm perform?  The function defaults to 2, but that seems to be over-sensitive for this data, so we set \`iter = 1\` by default in \`detect\_outliers()\`.  This can be changed via \`ts.iter\` in \`process\_plate()\`.

-   Background subtraction
    -   Should outlier readings in blank wells be removed before aggregating background measurements?  \`exclude.outliers\` defaults to TRUE in \`subtract\_background()\`; this can be changed via \`exclude.bg.outliers\` in \`process\_plate()\`.
    
    -   Should blank wells' absorbance readings be aggregated by mean or median for background subtraction?  \`process\_plate()\` passes "median" to \`basis\` in \`subtract\_background()\` by default; this can be changed via \`bg.basis\`.
    
    -   What's the minimum number of blank wells with data that are required for background subtraction to proceed at a given timepoint?  \`threshold\` defaults to 3 in \`subtract\_background()\`; this can be changed via \`bg.threshold\` in \`process\_plate()\`.

-   Plotting
    -   Should data from wells of the "ignore" type be plotted?  Even in empty wells, variation in readings can help identify problematic timepoints, so an initial analysis might want to show the raw data from these wells even though it's not otherwise informative; for a polished plot, these wells should generally be omitted.  \`show.ignores\` defaults to FALSE in \`plot\_abs()\`; this can be changed with \`show.ignores\` in \`process\_plate()\`.
    
    -   How many wells with outliers is too many for the data to be shown?  Widespread outliers can happen when something external affects the whole plate, e.g., at moments when the temperature changes or in recovery from those changes.  When many wells have outliers at the same timepoint, we may want to treat every measurement from that time as an outlier.  \`threshold\` defaults to 96 (i.e., any number of outliers is acceptable) in \`plot\_abs()\`; this can be changed via \`plot.threshold\` in \`process\_plate()\`.
    
    -   How should outliers be treated?  They can be dropped altogether, from points and lines; they can be dropped from the points but kept in the lines, so that they are visible but deemphasized; or they can be included fully.  \`process\_plate()\` passes "pointless" to \`dropstyle\` in \`plot\_abs()\`; this can be changed via \`plot.dropstyle\` in \`process\_plate()\`.
    
    -   Should background-subtracted or raw values be shown in test wells?  \`use.corr\` defaults to TRUE in \`plot\_abs()\`; this can be adjusted with \`use.corr\` in \`process\_plate()\`.  When \`use.corr = TRUE\`, background-subtracted values are shown for test wells, while raw values are shown for blank wells (and "ignore" wells, if plotted), with the aggregate background measure overlaid on these wells for a visual reference.  When \`use.corr = FALSE\`, only raw values are shown across the entire plate.
    
    -   What should the plot be titled?  Adding an informative title within the plotting panel, not just in the filename, helps track the provenance of figures arising from multiple experiments.  \`process\_plate()\` requires a \`plot.title\` parameter, which is then passed to \`plot\_abs()\`.  Similarly, \`expt.dates\` will feed into the plot's subtitle.  Data-analysis and plotting choices are automatically logged in the plot caption.
